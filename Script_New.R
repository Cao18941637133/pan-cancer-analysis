...
#asdasd----
MutIndex <- Sys.glob("~/OneDrive/离线数据/TCGADat/OriDat/Mutect2BioLink/*")
MutDat <- lapply(MutIndex, \(x){
    MutDat <- data.table::fread("~/OneDrive/离线数据/TCGADat/OriDat/Mutect2BioLink/UCEC.maf.gz",data.table = F)
    MutDat1 <- MutDat[MutDat$Hugo_Symbol %in% geneFamily1,]
    MutDat1 %>% dplyr::select(Hugo_Symbol,Tumor_Sample_Barcode,Variant_Classification)
})
hf(MutDat[[1]])
MutDatMerge <- do.call(rbind, MutDat)
hf(MutDatMerge)
MutDatMerge$Tumor_Sample_Barcode <- 
    str_sub(MutDatMerge$Tumor_Sample_Barcode, 1, 15)
PanTpm <- data.table::fread("~/OneDrive/离线数据/TCGADat/OriDat/tcga_Kallisto_tpm",nrows = 2)
hf(PanTpm)
Sample <- intersect(colnames(PanTpm), unique(MutDatMerge$Tumor_Sample_Barcode))
head(Sample)
length(Sample)
ColPindex <- match(Sample, colnames(PanTpm))
head(ColPindex)
ColPindex <- c(1, ColPindex)

library(magrittr)
MutDatMerge %<>% dplyr::filter(Tumor_Sample_Barcode %in% Sample)
Sample <- intersect(colnames(PanTpm), unique(MutDatMerge$Tumor_Sample_Barcode))
MutDatMerge %<>% dplyr::distinct()
Target <- MutDatMerge %>% group_by(Hugo_Symbol, Variant_Classification) %>% 
    count() %>% dplyr::filter(n >= 3) %>% 
    mutate(a = paste0(Hugo_Symbol,"_", Variant_Classification)) %>% 
    pull(a)
MutDatMerge$Group <- paste0(MutDatMerge$Hugo_Symbol, "_", MutDatMerge$Variant_Classification)
MutDatMerge1 <- MutDatMerge[MutDatMerge$Group %in% Target,]
MutDatMerge1 %>% group_by(Hugo_Symbol, Variant_Classification) %>% 
    count() %>% summary()
dim(MutDatMerge1)
Sample <- intersect(colnames(PanTpm), unique(MutDatMerge1$Tumor_Sample_Barcode))
ColPindex <- match(Sample, colnames(PanTpm))
max(ColPindex)
ColPindex <- c(1, ColPindex)
ColPindex <- c(ColPindex, 10485:10536)
PanTpm1 <- data.table::fread("~/OneDrive/离线数据/TCGADat/OriDat/tcga_Kallisto_tpm",select = ColPindex,
                             data.table = F)
idmap <- read.delim("~/project/gencode.v23.metadata.HGNC",header = F)
idmap <- idmap[!duplicated(idmap$V1),]
hf(idmap)
idmapId <- idmap$V1[idmap$V2 %in% geneFamily1]
rownames(idmap) <- idmap$V1
length(idmapId)
PanTpm2 <- PanTpm1[PanTpm1$sample %in% idmapId,]
PanTpm2 <- ColToRow(PanTpm2, "sample")
hf(PanTpm2)
PanTpm2$gsm <- idmap[rownames(PanTpm2),]$V2
head(PanTpm2$gsm)
anyDuplicated(PanTpm2$gsm)
PanTpm2 <- PanTpm2[!duplicated(PanTpm2$gsm),]
PanTpm2 %<>% ColToRow("gsm")
dim(PanTpm2)
hf(PanTpm2)
hf(MutDatMerge1)
MutAnalysis <- lapply(MutDatMerge1$Group, \(x){
    TargetSample <- MutDatMerge1$Tumor_Sample_Barcode[MutDatMerge1$Group == x]
    Sample1 <- intersect(TargetSample,colnames(PanTpm2))
    Len <- length(Sample1)
    Index <- match(Sample1, colnames(PanTpm2))
    Gene <- unique(MutDatMerge1$Hugo_Symbol[MutDatMerge1$Group == x])
    ExprDat <- PanTpm2[Gene,c(Sample1, colnames(PanTpm)[(10536-2*Len):10536])] %>% t() %>% as.data.frame()
    ExprDat <- ExprDat %>% rownames_to_column("Sample") %>% 
        dplyr::mutate(Group = ifelse(Sample %in% Sample1, x, "Non"))
    aa <- ExprDat %>% group_by(Group) %>% summarise(summmary = paste0(round(mean(!!sym(Gene)),2), "±",
                                                                      round(sd(!!sym(Gene))),2))
    Test <- wilcox.test(ExprDat[[Gene]]~ExprDat[["Group"]])
    Result <- data.frame(Group = x, Mut = aa$summmary[1], Non = aa$summmary[2], P.val = Test$p.value)
    return(Result)
})
MutAnalysisDat <- do.call(rbind, MutAnalysis)
MutAnalysisDat %<>% separate(Group,into = c("Gene", "Type"), sep = "_")
write.csv(MutAnalysisDat, "MutAnalysisDat.csv", row.names = F, quote = F)

# copy number ---------------------------------------------------------------------------------
CNVIndex <- Sys.glob("~/OneDrive/离线数据/TCGADat/OriDat/CopyNumberDat/CNVDatCategory/*")
CNVDat <- lapply(CNVIndex, \(x){
    CopyDat <- read.delim(x,
                          row.names = 1) 
    CopyDat1 <- CopyDat[geneFamily1,] %>% t() %>% as.data.frame()
    return(CopyDat1)
})
CNVDatDat <- do.call(rbind, CNVDat)
rownames(CNVDatDat) <- str_replace_all(rownames(CNVDatDat), "[.]", "-")
CNVINterSample <- intersect(rownames(CNVDatDat), colnames(PanTpm))
length(CNVINterSample)
CNVInterIndex <- match(CNVINterSample, colnames(PanTpm))
CNVInterIndex <- c(1, CNVInterIndex)
PanTpm1 <- data.table::fread("~/OneDrive/离线数据/TCGADat/OriDat/tcga_Kallisto_tpm",select = CNVInterIndex,
                             data.table = F)
idmap <- read.delim("~/project/gencode.v23.metadata.HGNC",header = F)
idmap <- idmap[!duplicated(idmap$V1),]
hf(idmap)
...
PanTpm2 <- PanTpm2[!duplicated(PanTpm2$gsm),]
PanTpm2 %<>% ColToRow("gsm")
PanTpm2 %<>% t() %>% as.data.frame()
CNVDatDat1 <- apply(CNVDatDat, 2, \(x){
    ifelse(x == 1, "Gain", 
           ifelse(x == 0, "Non", "Loss"))
})
CNVDatDat1 <- as.data.frame(CNVDatDat1)
hf(CNVDatDat1)
hf(PanTpm2)
x <- geneFamily1[1]
CNVAnalysis <- lapply(geneFamily1,\(x){
    JoinDat <- cbind(CNVDatDat1[CNVINterSample,x], PanTpm2[CNVINterSample, x])
    colnames(JoinDat) <- c("CNV", "Expr")
    JoinDat <- as.data.frame(JoinDat)
    JoinDat$Expr <- as.numeric(JoinDat$Expr)
    SumDat <- JoinDat %>% group_by(CNV) %>% 
        summarise(summmary = paste0(round(mean(Expr),2), "±",
                                    round(sd(Expr)),2))
    Test <-  kruskal.test(JoinDat$Expr ~ JoinDat$CNV)
    Result <- data.frame(Gene = x, Gain = SumDat$summmary[1],
                         Loss = SumDat$summmary[3], Non = SumDat$summmary[2],
                         P.val = Test$p.value)    
        
})
head(CNVAnalysis[[2]])
write.csv(do.call(rbind, CNVAnalysis), "CNVAnalysis.csv", row.names = F, quote = F)
